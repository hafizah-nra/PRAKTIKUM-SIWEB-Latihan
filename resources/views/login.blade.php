      ...
      <div class="login-card-body">
        ...
        @if($errors->has('login'))
          <div class="alert d-flex align-items-center gap-2 mb-4 py-2 px-3"
            ...
            <i class="bi bi-x-circle-fill me-2"></i>{{ $errors->first('login') }}
          </div>
        @endif

        <form method="POST" action="{{ route('login.proses') }}" autocomplete="off">
          @csrf
          ...
          <div class="login-field">
            ...
            <div class="login-input-wrap">
              <input type="text" id="username" name="username"
                ...
                value="{{ old('username', $_COOKIE['username'] ?? '') }}"
                required/>
              <span class="icon">
          ...
          <div class="login-field login-row-check">
            <label class="login-check-label">
              <input type="checkbox" name="remember"
                {{ isset($_COOKIE['username']) && $_COOKIE['username'] !== '' ? 'checked' : '' }}>
